...
from alpa.shard_parallel.pb_config import PBConfig
...
class AutoShardingOption:
    ...
    # # Parallel block opt
    # parallel_block_opt = True
    # # Hand specific parallel block
    # profile_config: ProfileConfig = None

    # print_strategy = False
    # debug_dir = ""
    # segment_profiling = False

    mode: str = ""
    pb_config = None
    seg_config_list = None
    print_strategy = False
    num_layers = -1
    batch_size = -1
    flop_count = -1
...
def run_auto_sharding_pass(
        ...
        solution_vector: Optional[Sequence[int]] = None):
    ...
    if (solution_vector is not None):
        load_s_val = True
        s_val = solution_vector

    parallel_block_opt = as_option.mode != "default"
    if (parallel_block_opt and len(as_option.pb_config.loaded_pb) > 0):
        print("Using Loaded Parallel Block and Parallel Plans...")
    ...
    with XlaPassContext({
            ...
            "auto_sharding::mode":
                as_option.mode,
            "auto_sharding::print_debug_info_to":
                as_option.debug_dir,
            "auto_sharding::parallel_block_opt":
                parallel_block_opt,
            "auto_sharding::loaded_pb":
                as_option.pb_config.loaded_pb,
            "auto_sharding::load_solution_vector":
                load_s_val,
            "auto_sharding::solution_vector":
                tuple(int(x) for x in s_val),
            "auto_sharding::num_layers":
                as_option.num_layers,
            "auto_sharding::bs_size":
                as_option.batch_size,
            "auto_sharding::forward_dot_count":
                as_option.pb_config.forward_dot_count,
            "auto_sharding::dot_pair_str":
                as_option.pb_config.dot_pair_str,
            "auto_sharding::merge_inst_str":
                as_option.pb_config.merge_inst_str,
            "auto_sharding::remove_dot_plan_str":
                as_option.pb_config.remove_dot_plan_str,
            "auto_sharding::add_dot_plan_str":
                as_option.pb_config.add_dot_plan_str,
            "auto_sharding::new_dot_plan_str":
                as_option.pb_config.new_dot_plan_str,
            "auto_sharding::layers_to_segment":
                tuple(int(x) for x in as_option.pb_config.layers_to_segment),
    }):
