...
def benchmark_func(sharded_exec,  warmup = 10, repeat = 3, number = 30):
    import time
    costs = []
    tic = time.time()
...
def benchmark_strategy(compiled, local_devices, device_inputs):
    costs = []
    def sharded_exec():
...
def test_exec(exec):
    ...
    if free_mem < exec.compiled.total_allocation_size() and free_mem != -1:   
        logging.warning(f"OOM: free_mem: {free_mem} compiled program need {exec.compiled.total_allocation_size()}")
        return cost_failed 
    # logging.info(f"need: {exec.compiled.total_allocation_size()}")       
    device_inputs = []
    try:
        for shape in input_shapes:
            device_inputs.append([
                exec.physical_mesh.backend.buffer_from_pyval(
                    np.empty(shape.dimensions(), shape.numpy_dtype()), device) for device in local_devices
            ])
        local_devices[0].synchronize_all_activity()
    except RuntimeError:
        costs = cost_failed
    costs = benchmark_strategy(exec.compiled, local_devices, device_inputs)
    return costs
...
def shard_parallel_internal(
    ...
    costs = test_exec(exec)
    print(costs)
    tflops = [(flop_count/1e12)/c for c in costs]
    print(f"TFLOP_COUNT: {flop_count/1e12}")
