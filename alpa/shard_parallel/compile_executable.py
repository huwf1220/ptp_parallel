...
from alpa.device_mesh import LogicalDeviceMesh, PhysicalDeviceMesh, VirtualPhysicalMesh
...
from alpa.shard_parallel.exp import (shard_parallel_internal_exp,
                                     shard_parallel_internal_seg_profiling,
                                     shard_parallel_internal_search,
                                     shard_parallel_internal_default,
                                     shard_parallel_internal_benchmark)
...
def compile_shard_executable(
    ...
    *avals: Sequence[AbstractValue],
    ...
    if num_micro_batches is None:
        if (as_option.mode == "profile"):
            return shard_parallel_internal_seg_profiling(
                fun, in_tree, out_tree_thunk, static_argnums, donated_invars,
                physical_mesh, logical_mesh_choices, as_option, *avals)
        elif (as_option.mode == "analysis"):
            return shard_parallel_internal_exp(fun, in_tree, out_tree_thunk,
                                               static_argnums, donated_invars,
                                               physical_mesh,
                                               logical_mesh_choices, as_option,
                                               *avals)
        elif (as_option.mode == "search"):
            return shard_parallel_internal_search(fun, in_tree, out_tree_thunk,
                                                  static_argnums,
                                                  donated_invars, physical_mesh,
                                                  logical_mesh_choices,
                                                  as_option, *avals)
        elif (as_option.mode == "benchmark"):
            return shard_parallel_internal_benchmark(
                fun, in_tree, out_tree_thunk, static_argnums, donated_invars,
                physical_mesh, logical_mesh_choices, as_option, *avals)
        else:
            return shard_parallel_internal_default(
                fun, in_tree, out_tree_thunk, static_argnums, donated_invars,
                physical_mesh, logical_mesh_choices, as_option, *avals)
    else:
...
def shard_parallel_internal(
    ...
    # Save search space and clustering info generated by alpa
    file_path = "/tmp/bert_2_1x4.csv"

    # Analyse and generate a more compact space that need to be profiled

    # Benchmark all solution vectors
    # with open(file_path, 'r', encoding="utf-8") as solution_vectors_csv:
    #     solution_vectors = csv.reader(solution_vectors_csv)
    #     for s_val in solution_vectors:
    #         hlo_ = copy.copy(hlo)
    #         # Compile a XLA executable
    #         hlo_, stage_plan = run_auto_sharding_pass_(hlo_, logical_mesh_choices[0],
    #                                                  "single", 1, as_option, s_val=s_val)
    ...
    #         # benchmark with profile method
    #         cost = exec.profile_with_dummy_inputs()
    #         print(cost)
