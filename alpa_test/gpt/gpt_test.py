...
class AutoShardingAttentionTest(unittest.TestCase):
    ...
    def run_gpt(self, batch_size, seq_len, num_layers, hidden_size,
    ...
    def test_gpt(self):
        state, hlo_ir, objective = self.run_gpt(self.batch_size, self.model_config.seq_len,
                                                self.num_layers, self.model_config.hidden_size,
                                                self.model_config.num_heads,
                                                False, False, self.device_mesh_1d)


def gpt_alpa_default_perf_test(config, batch_size, num_layer, device_num, data_type, debug_dir=""):
    gpt_test = AutoShardingAttentionTest()
    gpt_test.setUp(config, batch_size, num_layer, device_num, data_type, debug_dir)
    gpt_test.test_gpt()
...
if __name__ == "__main__":
    ...
    print(f"============ Alpa GPT test: config:{args.config} batchsize:{args.batch_size} layers:{args.num_layer} devices:{args.device_num} ============")
    gpt_alpa_default_perf_test(args.config, args.batch_size, args.num_layer, args.device_num, data_type, args.debug_dir)
