...
class AutoShardingAttentionTest(unittest.TestCase):
    ...
    def run_gpt_layers(self, batch_size, seq_len, num_layers, hidden_size,
    ...
    def test_gpt_layer_1d_mesh_pb(self, mode):
        self.as_option.mode = mode
        self.as_option.pb_config.forward_dot_count = 6 * self.num_layers
        state = self.run_gpt_layers(self.batch_size, self.model_config.seq_len,
                                    self.num_layers,
...
def gpt_pb_analysis_profiling_passes(mode,
                                     config,
                                     batch_size,
                                     num_layer,
                                     device_num,
                                     ...
                                     enum_and_bench,
                                     debug_dir=""):
    gpt_test = AutoShardingAttentionTest()
    gpt_test.setUp(config, batch_size, num_layer, device_num, data_type,
                   debug_dir)
    gpt_test.test_gpt_layer_1d_mesh_pb(mode)
...
if __name__ == "__main__":
    ...
    if args.mode == "analysis":
        print("============" * 10)
        print(
            f"============ GPT test: parallel_block_opt:{args.parallel_block_opt} config:{args.config} batchsize:{args.batch_size} layers:{args.num_layer} devices:{args.device_num} ============"
        )
    gpt_pb_analysis_profiling_passes(args.mode, args.config, args.batch_size,
