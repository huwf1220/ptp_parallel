...
inputs = torch.rand(cmd_args.batch, config.seq_len, config.hidden_size).to(device)
...
if local_rank == 0:
    print("***")
    print("***")
    print(f'batch_size: {cmd_args.batch} llama done',f'valid time={be_time - fs_time}',f'Average time={total_time / (cmd_args.iter / 2)}')
    print("***")
    print("***")