...
memory_efficient_pb_list = [2, 1, 0, 2]
memory_cost_banlance_pb_list = [0, 1, 0, 2]
cost_efficient_pb_list = [0, 1, 1, 1]
...
class AutoShardingLlamaTest(unittest.TestCase):
    ...
    def get_enum_bench_config(self):
        ...
        self.as_option.pb_config.forward_dot_count = 9 * self.model_config.n_layers
        self.as_option.pb_config.force_search_space = []

        if self.as_option.num_layers < 5:
            pb_list = cost_efficient_pb_list * self.as_option.num_layers
        elif self.as_option.num_layers == 5 and self.as_option.batch_size > 96:
            pb_list = memory_cost_banlance_pb_list * self.as_option.num_layers
        elif self.as_option.num_layers == 6:
            if self.as_option.batch_size < 80:
                pb_list = cost_efficient_pb_list * self.as_option.num_layers
            else:
                pb_list = memory_efficient_pb_list + memory_cost_banlance_pb_list * (
                    self.as_option.num_layers - 1)
        else:
            pb_list = memory_efficient_pb_list * self.as_option.num_layers

        # pb_lists = []
        # pb_lists.append( [2, 1, 0, 2] * 1 + [0, 1, 0, 2] * 5 ) # bs128 6layer
        # #pb_lists.append([0, 1, 0, 2] * 5) # bs128 5 layer
        # #pb_lists.append([0, 1, 0, 2] * 6 ) # bs 112 6layer
        self.as_option.pb_config.force_pb_lists = [pb_list]


def llama_pb_analysis_profiling_passes(model_setting, batch_size, num_layers,
                                       device_num, data_type,
    ...
    llama_test.setUp(model_setting, batch_size, num_layers, device_num,
                     data_type, debug_dir)
    llama_test.get_enum_bench_config()
    llama_test.test_llama_1d("benchmark")
...
if __name__ == "__main__":
    ...
    llama_pb_analysis_profiling_passes(args.d_model, args.batch_size,
                                       args.num_layer, args.device_num,
                                       data_type, args.parallel_block_opt,
                                       args.enum_and_bench, args.debug_dir)

############### 4096 6layer 128 ###############
#sval [0, 1, 0, 2, 0, 1, 0, 2] [0.27678311 0.27639961 0.27524829] 8768491972
#sval [2, 0, 0, 2, 2, 0, 0, 2] [0.42343783 0.42394125 0.42311823] 8228614596
#sval [2, 1, 0, 2, 2, 1, 0, 2] [0.38798702 0.38913345 0.38752913] 8189710404
#sval [2, 2, 0, 2, 2, 2, 0, 2] [0.47032034 0.47057605 0.47309673] 8019908284
#sval [0, 1, 1, 1, 0, 1, 1, 1] [0.2217164  0.22722423 0.22236192] 11646061880

############### 4096 6layer 112 ###############
#sval [0, 1, 0, 2, 0, 1, 0, 2] [0.24681568 0.2471211  0.24763346] 8301785668
#sval [1, 1, 0, 2, 1, 1, 0, 2] [0.3787452 0.3791846 0.3773942] 8282594028
#sval [1, 2, 0, 2, 1, 2, 0, 2] [0.38316011 0.38370442 0.3847599 ] 8194160988
#sval [2, 0, 0, 2, 2, 0, 0, 2] [0.36710227 0.36693585 0.36720836] 758269178
#sval [2, 2, 0, 2, 2, 2, 0, 2] [0.40932369 0.40770447 0.40842533] 7404910268
#sval [0, 1, 1, 1, 0, 1, 1, 1] [0.20976329 0.20998859 0.20834637] 11259919800
