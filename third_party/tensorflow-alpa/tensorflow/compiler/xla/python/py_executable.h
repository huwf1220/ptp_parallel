...
class PyLoadedExecutable
 ...
 public:
  ...
  void ChangeClient(const std::shared_ptr<PyClient>& new_client){
    this->client_ = new_client;
    PjRtStreamExecutorExecutable* exec_ptr = dynamic_cast<PjRtStreamExecutorExecutable*>(this->mutable_pjrt_executable());
    exec_ptr->set_new_client(dynamic_cast<PjRtStreamExecutorClient*>(new_client->pjrt_client()));
}
 private:
