...
// priority(operand) = max(x.output_spec.num_tiles for x in
// operand.strategies)
//
// Return `tie == True` if there are two operands with very close priorities
// and we cannot decide which one to follow.
...
BuildStrategyAndCost(const HloInstructionSequence& sequence,
                     ...
                     const AliasMap& alias_map,
                     const ClusterEnvironment& cluster_env,
                     AutoShardingSolverOption& solver_option) {//TODO[HUWF]
  ...
  for (size_t instruction_id = 0; instruction_id < instructions.size();
    ...
    switch (opcode) {
      ...
      case HloOpcode::kRng: {
        ...
        // If force_batch_dim_to_mesh_dim is set, filter out invalid
        // strategies and only keep the data parallel strategies.
      ...
      case HloOpcode::kBroadcast: {
        ...
        // If the operand is a scalar, following it only generates
        // "Replicated" strategy. So we should register new strategies instead
        // of following it.
        if (operand->shape().rank() == 0) {
      ...
      case HloOpcode::kReshape: {
        ...
        // [HUWF]force set some reshape to follow operand for llama 1D case.
        // bool special_reshape_for_llama = true;
        bool reshape_simple_follow = true;
        // if not input reshape for Q proj and K proj
        // if not reshape with a pattern of reshape->multiply with a broadcast.

        // [HUWF 20240821] remove this
        // std::set<int> qk_input_reshape_id;
        // for (int inst_id : tmp_for_llama) {qk_input_reshape_id.insert(inst_id);}
        // if (qk_input_reshape_id.count(instruction_id) != 0){
        //   special_reshape_for_llama = false;
        // }
        // [HUWF 20240821] add code for input reshapes in LLAMA and MoE
        // Dot not make some reshapes follow forward dot to let them follow dot in its PB later.
        // reshape->transpose->reshape->dot patterns in MoE.
        if(ins->users()[0]->opcode()==HloOpcode::kTranspose){
          const HloInstruction* next_user = ins->users()[0]->users()[0];
          if (next_user->opcode()==HloOpcode::kReshape 
          && next_user->users()[0]->opcode()==HloOpcode::kDot){
            //std::cerr << "[Reshape]->Transpose->Reshape->Dot, INST ID " << instruction_id << "\n";
            reshape_simple_follow = false;
          }
        }
        if(operand->opcode()==HloOpcode::kTranspose && ins->users()[0]->opcode()==HloOpcode::kDot){
          //std::cerr << "Transpose->[Reshape]->Dot, INST ID " << instruction_id << "\n";
          reshape_simple_follow = false;
        }

        const HloInstruction* user_ = instructions[instruction_id+1]; 
        if ((user_ != nullptr) && (operand->opcode()== HloOpcode::kAdd || operand->opcode()== HloOpcode::kTranspose)
            &&(user_->opcode() == HloOpcode::kMultiply)){
              reshape_simple_follow = false;

        }
        if (!solver_option.parallel_block_opt) {
          reshape_simple_follow == false;
        }
        ...
        if (!undefined_set.count(operand) &&
            ((ins->users().size() == 1 && !IsBatchDimSwitchReshape(ins)) ||
             (mesh_nn_dims >= 2 && !solver_option.allow_mixed_mesh_shape) || reshape_simple_follow)) {  //
          const StrategyVector* src_strategies = strategy_map.at(operand).get();
          CHECK(!src_strategies->is_tuple);
          strategies->following = src_strategies;
  ...
  for (HloInstruction* inst : instructions) {
    ...
    if (strategies->is_tuple) {
      ...
      if (inst->opcode() == HloOpcode::kTuple || IsPassThroughTuple(inst)) {
        ...
        // (because this update is very hard to do). This leads to
        // inconsistency between one operand's sharding and the sharding in
...
void GetCommVolume(const HloInstructionSequence& sequence, 
  ...
  for (size_t i = 0; i < N; ++i) {
    ...
    if (stra.communication_cost > 0) {
      total_volume += stra.communication_cost;
    }
  }
  ...
  std::cerr << "Comm Volume: " << total_volume << "\n";
...
#define DEBUG_INFO(out_, fn, prefix, file_name, needdump)      \
  if (needdump) {                                              \
    std::string file_path = prefix + "/" + file_name + ".txt"; \
    out_.open(file_path, std::ios::out | std::ios::trunc);     \
    out_ << fn;                                                \
    out_.close();                                              \
  }

static bool isVectorInCSV(std::vector<long int> v, std::string filename) {
  // Open the CSV file for reading
  std::ifstream csvFile;
  csvFile.open(filename);

  // Read each line in the file and compare it to the vector
  std::string line;
...
static void WriteToCSV(std::vector<long int> v, std::string filename) {
  // Check if the vector is already in the CSV file
  if (isVectorInCSV(v, filename)) {
    std::cout << "Vector is already in the CSV file." << std::endl;
    return;
  }
  ...
  // If the file is empty, write the vector as the first line
  if (csvFile.tellp() == 0) {
    for (int i = 0; i < v.size(); i++) {
      csvFile << v[i];
      if (i != v.size() - 1) csvFile << ",";
    }
    csvFile << std::endl;
    first_line = true;
  }
  ...
  if (!first_line) {
    // Print and write the values of the vector
    for (int i = 0; i < v.size(); i++) {
      std::cout << v[i] << " ";
      csvFile << v[i];
      if (i != v.size() - 1) csvFile << ",";
...
StatusOr<bool> AutoSharding::Run(
  ...
  ///////////////////////////////////////////////////////////////////////
  solver_option.bs_size = pass_context::GetInt("auto_sharding::bs_size", -1);
  solver_option.num_layers = pass_context::GetInt("auto_sharding::num_layers", -1);
  solver_option.mode = pass_context::GetString("auto_sharding::mode", "default");
  solver_option.parallel_block_opt =
      pass_context::GetBool("auto_sharding::parallel_block_opt", false);
  int forward_dot_count =
      pass_context::GetInt("auto_sharding::forward_dot_count", -1);
  std::string loaded_pb =
      pass_context::GetString("auto_sharding::loaded_pb", "");
  std::string dot_pairs_str =
      pass_context::GetString("auto_sharding::dot_pair_str", "");
  std::string merge_inst_str =
      pass_context::GetString("auto_sharding::merge_inst_str", "");
  std::string remove_dot_plan_str =
      pass_context::GetString("auto_sharding::remove_dot_plan_str", "");
  std::string add_dot_plan_str =
      pass_context::GetString("auto_sharding::add_dot_plan_str", "");
  std::string new_dot_plan_str = 
      pass_context::GetString("auto_sharding::new_dot_plan_str", "");
  std::string new_inst_plan_str = 
      pass_context::GetString("auto_sharding::new_inst_plan_str", "");
  // solver_option.comm_free_opt =
  //     pass_context::GetBool("auto_sharding::comm_free_opt", false);
  // solver_option.handle_input_param =
  //     pass_context::GetBool("auto_sharding::handle_input_param", false);
  // solver_option.handle_non_comm_free_elemwise = pass_context::GetBool(
  //     "auto_sharding::handle_non_comm_free_elemwise", false);
  // solver_option.set_key_op_following =
  //     pass_context::GetBool("auto_sharding::set_key_op_following", false);
  bool dump_debug_info =
      pass_context::GetBool("auto_sharding::print_strategy", false);
  std::string dump_to =
      pass_context::GetString("auto_sharding::print_debug_info_to", "");

  std::ofstream out_stream;
  if (dump_debug_info) {
    tsl::Env::Default()->RecursivelyCreateDir(dump_to);
  }

  DEBUG_INFO(out_stream, module->ToString(), dump_to, "before_ir",
              dump_debug_info);
  ...
  // configure parallel block by hand specification.
  ParallelBlockConfig pb_config(dot_pairs_str, merge_inst_str, remove_dot_plan_str, add_dot_plan_str, new_dot_plan_str, new_inst_plan_str, forward_dot_count);
  
  // // [fixed]some but not all reshape operands in llama should to be set to follow its operand, we temporily pickup some reshape to avoid following.
  // std::vector<int> new_inst_plan;
  // for (auto p : pb_config.GetNewInstPlan()){
  //   new_inst_plan.push_back(p.first.second);
  // }

  TF_ASSIGN_OR_RETURN(
      std::tie(strategy_map, leaf_strategies, associative_dot_pairs),
      BuildStrategyAndCost(sequence, ins_depth_map, batch_dim_map, alias_map, cluster_env, solver_option));

  DEBUG_INFO(out_stream, PrintStrategyMap(strategy_map, sequence), dump_to,
             "strategy_map", dump_debug_info);
  std::vector<int> origin_stra_num;
  for (const auto& stra_vec : leaf_strategies) {
    origin_stra_num.push_back(stra_vec->leaf_vector.size());
  }

  std::vector<ParallelBlock> parallel_blocks;
  ReindexItemList reindex_arr;
  std::vector<int> set_to_one;
  LOG(INFO) << "Building PB...\n";
  if (solver_option.parallel_block_opt) {
    // Based on the independently generated strategies of each operator, we
    // use fine-grained dependencies between inst to help us identify some
    // operator combinations, whose parallelization plans is less than the
    // combination of their individual strategies.
    LOG(INFO) << "Building PB...\n";
    BuildParallelBlocks(sequence, leaf_strategies, strategy_map, cluster_env,
                        pb_config, parallel_blocks, reindex_arr, solver_option, set_to_one);
    if (!loaded_pb.empty()) {
      LoadParallelBlocks(loaded_pb, sequence, parallel_blocks);
    }

    LOG(INFO) << "Before Reset StrategyMap\n";
    // Reset strategies for each inst in parallel blocks, the number of
    // strategies for the dst instructions of each parallel block may increase
    // or decrease, while all other instructions in this block maintain the same
    // number and order of strategies as dst. In this way, we can utilize the
    // "following" attribute originally used by Alpa to merge cost graph nodes
    // to help us complete instruction clustering, avoiding the need to add
    // our code during Alpa's cost graph construction, simplification, and ILP
    // solving.
    //set_to_one.clear();
    ResetStrategiesForParallelBlocks(sequence, parallel_blocks, leaf_strategies,
                                     strategy_map, set_to_one, solver_option);
    LOG(INFO) << "After Reset StrategyMap\n";
  }
  AliasSet alias_set =
      BuildAliasSet(module, alias_analysis->dataflow_analysis(), strategy_map);

  // ----- Build cost graph and merge unimporant nodes -----
  auto cost_graph_build_start = std::chrono::steady_clock::now();
  CostGraph cost_graph(leaf_strategies, associative_dot_pairs);
  auto cost_graph_build_overhead =
      std::chrono::duration_cast<std::chrono::milliseconds>(
          std::chrono::steady_clock::now() - cost_graph_build_start);

  LOG(INFO) << "****Built Cost Graph*****\n";
  ///
  // DEBUG_INFO(out_stream, cost_graph.ToString(), dump_to, "cost_graph",
  //            dump_debug_info);

  std::vector<int> failed_merged;
  if (solver_option.parallel_block_opt) {
    LOG(INFO) <<"Before RuleBasedSimplifyAfterStrategiesReorder\n"; 
    cost_graph.RuleBasedSimplifyAfterStrategiesReorder(
        sequence, leaf_strategies, parallel_blocks, origin_stra_num);
    LOG(INFO) <<"Before ReduceEdgesForParallelBlocks\n"; 
    ReduceEdgesForParallelBlocks(sequence, leaf_strategies, strategy_map,
                                 cost_graph, parallel_blocks, failed_merged);
    DEBUG_INFO(out_stream, cost_graph.ToString(), dump_to,
               "pb_cost_graph_simplify", dump_debug_info);
  } else {
    cost_graph.Simplify();
    DEBUG_INFO(out_stream, cost_graph.ToString(), dump_to,
               "cost_graph_simplify", dump_debug_info);
  }
  // first pass, save all analysed info.
  if (solver_option.parallel_block_opt && 
            (solver_option.mode=="analysis_global" || solver_option.mode=="analysis_segment" || solver_option.mode == "search")){
    SaveReindexFile(sequence, cost_graph, leaf_strategies, strategy_map, 
                  dump_to, failed_merged, parallel_blocks, reindex_arr);
  }
  
  if (solver_option.parallel_block_opt && solver_option.mode=="analysis_global"){
    std::vector<ParallelBlock> pbs;
    std::vector<int> pbs_idx;
    std::vector<std::pair<int, int>> interval_list;
    for (int i = 0; i < parallel_blocks.size(); ++i){
      if (parallel_blocks[i].first.size() != 0) {
        pbs.push_back(parallel_blocks[i]);
        pbs_idx.push_back(i);
      } 
    }
    std::vector<int64_t> layers_to_segment = pass_context::GetIntVector("auto_sharding::layers_to_segment");
    if(layers_to_segment[0]!=-1){
      int current_pb = 0;
      for (auto seg_length: layers_to_segment){
        interval_list.push_back(std::pair<int,int>(current_pb, current_pb+seg_length));
        current_pb = current_pb+seg_length;
      }
    }
    else {
      ExtractPBSequences(sequence, pb_config, ins_depth_map, solver_option, pbs, pbs_idx, interval_list);   
    }
    
    SaveSegmentMapping(interval_list, dump_to);
  } 

  // ----- Call the ILP solver -----
  LOG(INFO) << "Solve based on ILP or infer based on PB...\n";
  ...
  if (!solver_option.load_solution_vector) {
    ...
    DEBUG_INFO(out_stream, PrintStrategyMap(strategy_map, sequence), dump_to,
             "default_strategy_map", dump_debug_info);
    auto search_overhead = std::chrono::duration_cast<std::chrono::milliseconds>(
      std::chrono::steady_clock::now() - search_start);
  } else {
    s_val = pass_context::GetIntVector("auto_sharding::solution_vector");
    if (solver_option.parallel_block_opt){
      InferSolutionForRemainInsts(sequence, leaf_strategies, strategy_map, cost_graph, parallel_blocks, s_val, dump_to);
      DEBUG_INFO(out_stream, PrintStrategyMap(strategy_map, sequence), dump_to,
             "profile_strategy_map", dump_debug_info);
    }
  }
  ...
  //CheckEdgeCosts(leaf_strategies, cost_graph, s_val);
  SetHloSharding(sequence, strategy_map, cost_graph, s_val, cluster_env,
                 solver_option);
  
  DEBUG_INFO(out_stream, PrintStrategyMap(strategy_map, sequence), dump_to,
             "pb_strategy_map", dump_debug_info);
  DEBUG_INFO(out_stream, DumpSVFile(cost_graph, strategy_map), dump_to, "reindexing", dump_debug_info);
  const char* dump_comm_volume_str = std::getenv("DUMP_COMM_VOLUME");
  if (dump_comm_volume_str != nullptr && std::string(dump_comm_volume_str) == "true"){
    GetCommVolume(sequence, leaf_strategies, cost_graph, s_val);
  }
  DEBUG_INFO(out_stream,
             PrintAutoShardingSolution(sequence, liveness_set, ins_depth_map,
                                       strategy_map, leaf_strategies,
                                       cost_graph, s_val, objective),
             dump_to, "auto_sharding_solution", dump_debug_info);
  DEBUG_INFO(out_stream, module->ToString(), dump_to, "after_ir",
            dump_debug_info);
  DEBUG_INFO(out_stream,
             DumpFollowingReindex(strategy_map, sequence, cost_graph), dump_to,
             "follow_idx_and_reindexing", dump_debug_info);
  DEBUG_INFO(out_stream,
             DumpNotFollowing(strategy_map, sequence, cost_graph, s_val,
                              failed_merged),
             dump_to, "not_follow_idx", dump_debug_info);
