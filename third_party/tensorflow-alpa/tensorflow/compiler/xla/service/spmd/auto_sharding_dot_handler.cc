...
class DotHandler {
 public:
  ...
  void SplitAtOneBatchDim(int64_t dim) {
    if (device_mesh.dim(0) == 1 || device_mesh.dim(1) == 1) {
      for (int64_t j = 0; j < device_mesh.num_dimensions(); ++j) {
        if (device_mesh.dim(j) == 1 ||
            ins->shape().dimensions(dim) < device_mesh.dim(j)) {
          continue;
        }

        std::string name = absl::StrFormat("Sb_%d = Sb x Sb @ {%d}", dim, j);
        HloSharding output_spec = Tile(ins->shape(), {dim}, {j}, device_mesh);
        HloSharding lhs_spec =
            Tile(lhs->shape(), {lhs_batch_dims[dim]}, {j}, device_mesh);
        HloSharding rhs_spec =
            Tile(rhs->shape(), {rhs_batch_dims[dim]}, {j}, device_mesh);
