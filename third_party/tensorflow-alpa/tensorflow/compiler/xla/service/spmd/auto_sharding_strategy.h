...
enum DotKind {
  ...
  FirstInDotPair = 0,
  SecondInDotPair = 1,
  BatchedMMBetweenDotPair = 2,
  SecondInDotPairSecondOperand = 3,
};
using ParallelPlans = std::vector<std::vector<int>>;
using ParallelBlock = std::pair<std::vector<HloInstruction*>, ParallelPlans>;
using DepsToDotMap = absl::flat_hash_map<const HloInstruction*, std::pair<int, std::vector<int>>>;
using DotPairs = std::vector<std::pair<std::pair<int, int>, std::vector<int>>>;
using InstsDepsByMultiPB =  std::vector<ParallelBlock>;

class ParallelBlockConfig {
public:
  ParallelBlockConfig() = default;
  ParallelBlockConfig(const std::string& dot_pairs_str,
                      const std::string& merge_inst_str,
                      const std::string& remove_dot_plan_str,
                      const std::string& add_dot_plan_str,
                      const std::string& new_dot_plan_str,
                      const std::string& new_inst_plan_str,
                      int forward_dot_count = 0){
    LoadDataToDotPairs(dot_pairs_str, merge_inst_str, remove_dot_plan_str, add_dot_plan_str, new_dot_plan_str, new_inst_plan_str);
    _forward_dot_count = forward_dot_count;
  }
  int _forward_dot_count;
  void LoadDotPairs(const std::string& str) {
    LoadData(str, dot_pairs_);
  }

  void LoadMergeInst(const std::string& str) {
    LoadData(str, merge_inst_);
  }

  void LoadRemoveDotPlan(const std::string& str) {
    LoadData(str, remove_dot_plan_);
  }

  void LoadAddDotPlan(const std::string& str) {
    LoadData(str, add_dot_plan_);
  }

  void LoadNewDotPlan(const std::string& str) {
    LoadData(str, new_dot_plan_);
  }

  void LoadNewInstPlan(const std::string& str) {
    LoadData(str, new_inst_plan_);
  }

  void LoadData(const std::string& str, DotPairs& data) {
    ...
    while (std::getline(iss, line, ',')) {
      ...
      int x, y;
      std::istringstream x_stream(elements[0]);
      x_stream >> x;
      std::istringstream y_stream(elements[1]);
      y_stream >> y;
      std::vector<int> values;
      for (size_t i = 2; i < elements.size(); i++) {
        int value;
        std::istringstream value_stream(elements[i]);
        value_stream >> value;
        values.push_back(value);
      }
      data.emplace_back(std::make_pair(std::make_pair(x, y), values));
    }
  }
  void LoadDataToDotPairs(const std::string& dot_pairs_str,
                          const std::string& merge_inst_str,
                          const std::string& remove_dot_plan_str,
                          const std::string& add_dot_plan_str, 
                          const std::string& new_dot_plan_str,
                          const std::string& new_inst_plan_str) {
    LoadDotPairs(dot_pairs_str);
    LoadMergeInst(merge_inst_str);
    LoadRemoveDotPlan(remove_dot_plan_str);
    LoadAddDotPlan(add_dot_plan_str);
    LoadNewDotPlan(new_dot_plan_str);
    LoadNewInstPlan(new_inst_plan_str);
  }
  const DotPairs& GetDotPairs() const {
    return dot_pairs_;
  }
  const DotPairs& GetMergeInst() const {
    return merge_inst_;
  }
  const DotPairs& GetRemoveDotPlan() const {
    return remove_dot_plan_;
  }
  const DotPairs& GetAddDotPlan() const {
    return add_dot_plan_;
  }
  const DotPairs& GetNewDotPlan() const {
    return new_dot_plan_;
  }
  const DotPairs& GetNewInstPlan() const {
    return new_inst_plan_;
  }

private:

  DotPairs dot_pairs_;
  DotPairs merge_inst_;
  DotPairs remove_dot_plan_;
  DotPairs add_dot_plan_;
  DotPairs new_dot_plan_;
  DotPairs new_inst_plan_;
};
...
class CostGraph {
 public:
  ...
  //
  void SimplifyParallelBlockInCostGraph(
      std::vector<std::pair<int, int>>& merge_pairs);
  void MergeNodeForParallelBlock(int src, int dst);
  void RuleBasedMergeNodeForPBdst(int src, int dst,
                                  const std::vector<int>& dst_parallel_plan,
                                  int dst_origin_stra_num);
  void RuleBasedSimplifyAfterStrategiesReorder(
      const HloInstructionSequence& sequence, const LeafStrategies& leaf_strategies,
