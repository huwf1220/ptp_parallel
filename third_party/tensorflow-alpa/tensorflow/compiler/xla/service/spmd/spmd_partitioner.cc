    ...
    if (!target.IsReplicated()) {
      ...
      LOG(ERROR)
          ...
          << " without doing a full rematerialization of the tensor. You "
             ...
             "this from happening. " << hlo_->ToString();
        pid_t processID = getpid();
        std::stringstream flagFileName;
        flagFileName << "/tmp/failed_compile_" << processID << ".txt";
        std::string fileName = flagFileName.str();
        std::ofstream flagFile(fileName, std::ios::out | std::ios::trunc);
        if (flagFile.is_open()) {
            flagFile << "True";  // 写入字符串
            flagFile.flush();
            flagFile.close();  // 关闭文件
