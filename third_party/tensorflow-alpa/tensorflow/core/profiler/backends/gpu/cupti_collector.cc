...
class PerDeviceCollector {
 private:
  ...
  void CreateXEvent(const CuptiTracerEvent& event, XPlaneBuilder* plane,
    ...
    if (event.start_time_ns < start_gpu_ns || event.end_time_ns > end_gpu_ns ||
      ...
      std::cerr << "events have abnormal timestamps:" << event.name
    ...
    if (kernel_name[0] == 'n' && kernel_name[1] == 'c' && event.device_id == 0) {
      float comm_time = float(event.end_time_ns - event.start_time_ns) / float(1000000000);
      //std::cerr << kernel_name << " " << comm_time<< "\n";
      comm_overhead += comm_time;
    }
    else {
      if (event.device_id == 0  && kernel_name!="cuStreamSynchronize") {
        float comp_time = float(event.end_time_ns - event.start_time_ns) / float(1000000000);
        comp_overhead += comp_time;
        //std::cerr << kernel_name << " " << comp_time<< "\n";
      }
    }
    if (event.source == CuptiTracerEventSource::DriverCallback) {
      xevent.AddStatValue(
 ...
 public:
  ...
  float comm_overhead = 0.0;
  float comp_overhead = 0.0;
  ...
  size_t Flush(uint64 start_gpu_ns, uint64 end_gpu_ns,
    ...
    if(device_id_ == 0) {
      std::cerr << "Comm Overhead: " << comm_overhead << "\n";
      std::cerr << "Comp Overhead: " << comp_overhead << "\n";
    }
